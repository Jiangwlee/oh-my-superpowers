...
- `read-url` 会自动识别 reddit.com、x.com、xueqiu.com、tgb.cn 的 URL 并路由到对应的站点读取工作流。淘股吧 URL 固定返回与 `taoguba read` 相同的 compact JSON。
...
| 淘股吧 / Taoguba / TGB | `omp web-operator taoguba login`、`omp web-operator search taoguba <query> [limit] --year YYYY [--sort hot]`、`omp web-operator taoguba read <url>`、`omp web-operator taoguba jinghua [hours] [limit]`、`omp web-operator taoguba following [hours] [limit]` |
