...
- `search` does not accept `--platform`, `--query`, or `--limit`.
- Taoguba search additionally requires `--year YYYY` and accepts
  `--sort hot|latest`; its default sort is `hot`.
- `--target` is available for browser-backed searches.
...
| Taoguba historical search | `omp web-operator search taoguba "1112 复盘" 10 --year 2024 --sort hot` |
...
- `--limit N`：非站点 URL 按字节截断；淘股吧按 Unicode 字符截断正文，默认 500，传 `0` 返回全文。
...
- `read-url` 会自动识别 reddit.com、x.com、xueqiu.com、tgb.cn 的 URL 并路由到对应的站点读取工作流。淘股吧 URL 固定返回与 `taoguba read` 相同的 compact JSON，并转发 `--limit`。
...
| 淘股吧 / Taoguba / TGB | `omp web-operator taoguba login`、`omp web-operator search taoguba <query> [limit] --year YYYY [--sort hot]`、`omp web-operator taoguba read <url> [--limit N]`、`omp web-operator taoguba jinghua [hours] [limit]`、`omp web-operator taoguba following [hours] [limit]` |
