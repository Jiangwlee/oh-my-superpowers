#!/usr/bin/env bash
# This file provides shared helpers for the web-operator weixin.sogou.com workflows.
...
# sogou_find_target, wait_for_url_contains, and wait_for_sogou_selector.
#
# sogou_find_target finds an existing weixin.sogou.com tab or creates a new one.
...
sogou_find_target() {
  ...
  find_or_create_tab "https://weixin.sogou.com" "weixin.sogou.com"
}
...
(async () => {
  ...
  throw new Error('Timed out waiting for Sogou selector');
})()
EOF
  expr="${expr/LIMIT_MS/${limit}}"
