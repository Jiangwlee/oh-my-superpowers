"""Shared helpers for kickoff scripts."""
...
def load_yaml(path: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise SystemExit(f"[kickoff] yaml not found: {path}")
    data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    if not isinstance(data, dict):
        raise SystemExit(f"[kickoff] expected mapping in yaml: {path}")
    return data
...
def require_story_dir(story_root: Path, story: str) -> Path:
    ...
    if story_dir is None:
        raise SystemExit(f"[kickoff] story not found under {story_root}: {story}")
    return story_dir
