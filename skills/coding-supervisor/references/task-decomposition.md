...
Read this in Phase 1 step 5 (Task Breakdown) and Phase 2 step 2 (JIT Spec).
...
|---|---|
| **Phase 1 step 5 — Breakdown (skeleton only)** | `tasks.yaml` entries: `id / title / wave / depends_on / files_modified（estimate）/ test_layer`. `spec: null`, no `tasks/task-NN.md` file. |
| **Phase 2 step 2 — JIT Spec (per wave)** | Copy `skills/coding-supervisor/templates/task.md` to `<PROJECT_ROOT>/stories/<YYYY-MM-DD>-<slug>/tasks/task-NN.md`. Fill `Objective / Protocol / Acceptance Checklist`. Set `spec: tasks/task-NN.md` in `tasks.yaml`. |

**Hard rule** (enforced by the `task update` CLI; full usage in SKILL.md Phase 2 step 2): flipping `status: executing` is rejected when `spec` is null/empty. Write the JIT spec first.

Before writing each wave's specs:

1. Read `story-memory.md` (mandated by Phase 2 step 1).
...
Every entry MUST set: `id`, `title`, `wave`, `depends_on`, `spec` (null at breakdown), `files_modified`, `test_layer`.

`test_layer` = the lowest layer that can falsify acceptance (Rule 1).

- **Direct-edit fields**: appending, reordering, or removing tasks is a direct `tasks.yaml` edit. The `omp` command handles only high-frequency state fields (status / worker / reviewer / commit / note).
