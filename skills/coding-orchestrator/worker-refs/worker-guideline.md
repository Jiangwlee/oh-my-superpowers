...
Behavioral protocol for sub-agents dispatched by the coding orchestrator. The orchestrator injects this into sub-agent context at dispatch time.
...
1. Follow `references/constitution.md` — simplicity, surgical changes, goal-driven.
...
4. After each file change, verify it compiles/parses before moving on.
...
2. On failure, debug using the guideline below (max 3 attempts).
3. On all pass, report completion.

</step>

## Analysis paralysis guard

**5+ consecutive Read/Grep/Glob calls without any Edit/Write/Bash = STOP.**

You are stuck. Take action:

1. Write one sentence stating why you have not written anything yet.
...
1. STOP fixing.
2. Document what you tried, what failed, and the current hypothesis.
...
**Scope boundary**: only auto-fix issues DIRECTLY caused by your current changes. Pre-existing bugs, linting warnings, and unrelated failures are out of scope — note them but do NOT fix them.
