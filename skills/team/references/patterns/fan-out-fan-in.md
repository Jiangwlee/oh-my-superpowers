...
```bash
# Fan-out：并行启动
omp dispatch run claude "从安全角度分析..." --output-file /tmp/security.md &
PID1=$!
omp dispatch run claude "从性能角度分析..." --output-file /tmp/performance.md &
PID2=$!
omp dispatch run pi "从成本角度分析..." --output-file /tmp/cost.md &
PID3=$!
wait $PID1 $PID2 $PID3
