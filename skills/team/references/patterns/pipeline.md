...
1. Orchestrator 按顺序调用 `omp dispatch run`，等待每步完成后再启动下一步
...
```bash
# Step 1: codex 实现
OUTPUT=$(omp dispatch run codex "实现 parse_config 函数..." --cwd /project)
if [ $? -ne 0 ]; then echo "Step 1 failed" >&2; exit 1; fi

# Orchestrator 中间决策：检查输出，构建 review prompt

# Step 2: claude 审查
REVIEW=$(omp dispatch run claude "审查以下代码的质量：${OUTPUT}")
