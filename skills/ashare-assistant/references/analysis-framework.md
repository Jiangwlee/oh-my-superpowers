...
> ⚠️ 以下是A股市场硬性规定，所有仓位建议必须严格遵守：
> 1. **最小交易单位**：A股买卖单位为"手"= 100股。目标仓位必须换算为股数，并向下取整至100的整数倍。
>    例：计算结果为230股 → 实际操作200股；结果为80股 → 不可操作（金额不足一手）
> 2. **最小减仓单位**：减仓也必须是100股整数倍。**当持仓≤100股时，不建议减仓操作**（无法拆分）；此时只能选择全部清仓或继续持有。
> 3. **减仓比例**：对持仓100股的个股提出"减仓X%"的建议是无效建议（无法执行），必须改为"清仓"或"持有"。
> 4. **股数计算示例**：目标仓位10000元 / 当前价25.5元 = 392股 → 向下取整至300股（3手）
...
- **目标仓位**：[基础仓位金额 × 深度分析乘数 = X元 → 当前价Y元/股 → Z股（向下取整至100整数倍）]
  （基础仓位来自 active.yaml trend_stock.position；深度分析乘数来自本条"深度分析校准"，无校准则乘1.0）
  ⚠️ 若计算结果不足100股，说明账户资金不足以建仓本股，建议降低目标仓位或跳过
