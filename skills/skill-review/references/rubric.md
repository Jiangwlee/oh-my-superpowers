...
- [ ] 格式按语义选型，不得混用：
  - 编号列表 `1. 2.`：严格顺序步骤（换序会破坏含义）
  - 无序列表 `-`：无顺序枚举（概念、选项、并列规则）
  - Checkbox `- [ ]`：**可勾选的完成项**——待办任务、验证清单、审查条目；每项必须独立可判定（done / not done）
  - 表格 `\| ... \|`：多维对比或属性映射（≥2 列信息）
  - Mermaid：见 Pipeline 第 13 条
- [ ] Checkbox `- [ ]` 使用规范：
  - 不得用于纯信息展示（已有状态字段时用表格或无序列表；如 `- [ ] <子问题>（状态：open|partial|answered）` 属反模式——状态已显式表达，checkbox 冗余）
  - 不得用于互斥分支（分支用"如果 X → Y；否则 → Z"显式写出）
  - 不得嵌套勾选框（最多一层 `- [ ]`；子项改用普通 bullet 或折行描述）
  - 同一 checklist 内句式统一：全部祈使（"检查 X"）/ 全部陈述（"X 存在"）/ 全部疑问（"X 是否成立？"）三选一，不得混用
