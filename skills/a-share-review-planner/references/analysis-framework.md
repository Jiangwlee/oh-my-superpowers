...
- [第三步：个股筛选](#第三步个股筛选) — 四因子评分、趋势候选 + 题材候选
...
对通过第一道的所有股票，按以下公式计算综合评分：

```
综合评分 = 趋势分 + 资金加分 + 题材加分 + 情绪加分 + 弱市惩罚
```

**① 趋势分**

取 `trend_scan.json` 中的 `score_total_100`（范围 0–100）。

**② 资金加分**（来自 `funding.json` → `trend_candidates_funding`，按代码匹配 `rank` 字段）

| 条件 | 加分 |
|------|------|
| rank ≤ 20（主力净流入全市场前20） | +6 |
| rank 21–100 | +3 |
| rank 101–300 | +1 |
| rank > 300 或 net_inflow ≤ 0 或不在列表中 | 0 |

> `trend_candidates_funding` 为空时，检查 `main_force_top20` 中是否存在该代码，存在则按 rank ≤ 20 计。
...
| 所属**新兴题材**，评估：高 | +3 |
| 所属**新兴题材**，评估：中 | +2 |
| 属于市场当日热门板块（但非已识别主线/新兴题材） | +1 |
| 无题材热度 | 0 |

**④ 情绪加分**（来自 `trend_scan.json` → `emotion_level`）

| 条件 | 加分 |
|------|------|
| L4 或 L5（且 trade_signal ≠ 卖出） | +2 |
| L3 | +1 |
| L1 或 L2 | 0 |

**⑤ 弱市惩罚**（仅当第一步判断 `market_regime == weak` 时启用）
...
**action 赋值规则**（写入 candidates.json）：

| 条件 | action |
|------|--------|
| strong + 资金加分 ≥ 3 + trade_signal == 买入 | `buy` |
| 已在当前持仓 + 未触发止损 | `hold` |
| 弱市 / 中性市场 + 非持仓 | `watch` |

> 弱市统一使用 `watch`，不产生 `buy`，除非有明确突破买点（需在报告中显式说明理由）。

#### 例外：主线题材强制纳入

若主线题材处于**启动或加速**阶段，且龙头股**不在趋势候选池**中（未通过 trend_report 硬门槛），可额外添加最多 1 只纯题材股，须在候选记录中标注来源为「题材主线强制纳入」，并说明为何未通过趋势筛选。
...
```
[代码] [名称] [类型：趋势/题材]
评分：趋势(X) + 资金(X) + 题材(X) + 情绪(X) + 惩罚(X) = X | rank=X net_inflow=X亿
选股理由：（1-2句核心逻辑，说明选它而不选其他股的理由）
风险点：（1-2个主要风险）
```
