...
def _order_price(order: dict[str, Any]) -> float:
    return _safe_float(order.get("price") or order.get("deal_price") or order.get("match_price"))
...
def _parse_position_limits(strategy: dict[str, Any]) -> dict[str, Any]:
    trend = strategy.get("trend_stock") if isinstance(strategy.get("trend_stock"), dict) else {}
    theme = strategy.get("theme_stock") if isinstance(strategy.get("theme_stock"), dict) else {}
    market_pos = strategy.get("market_position") if isinstance(strategy.get("market_position"), dict) else {}
    return {
...
def _filled_orders(orders: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [o for o in orders if isinstance(o, dict) and not _is_cancelled(o) and _is_filled(o)]
...
def _analyze_timing(orders: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    codes = sorted({_norm_code(o.get("code")) for o in filled if _norm_code(o.get("code"))})
    if codes:
    ...
    for order in filled:
        ...
        if direction == "buy":
            if pos_pct > 0.80:
                flaws.append(
                    _flaw(category="timing_flaw", severity="error", code=code, name=name, message="严重追高")
                )
            elif pos_pct > 0.67:
                flaws.append(
                    _flaw(category="timing_flaw", severity="warning", code=code, name=name, message="追高买入")
                )
            ...
            if pos_pct < 0.20:
                flaws.append(
                    _flaw(category="timing_flaw", severity="error", code=code, name=name, message="严重恐慌卖出")
                )
            elif pos_pct < 0.33:
                flaws.append(
                    _flaw(category="timing_flaw", severity="warning", code=code, name=name, message="恐慌卖出")
                )
...
def _check_position_compliance(
    ...
    hold_list = positions.get("hold_list") if isinstance(positions.get("hold_list"), list) else []

    total_position_pct = ((total_assets - usable_cash) / total_assets * 100.0) if total_assets > 0 else 0.0
    ...
    regime_range = market_ranges.get(regime) if isinstance(market_ranges, dict) else None
    regime_range_label = ""
    ...
    check = {
        ...
        "compliant": not any(f["category"] == "position_flaw" and f["severity"] in {"warning", "error"} for f in flaws),
    }
    return flaws, check
...
def _holding_days_from_history(code: str, history: dict[str, Any]) -> int:
    ...
    for date in sorted(positions_map.keys(), reverse=True):
        ...
        for pos in hold_list if isinstance(hold_list, list) else []:
            if isinstance(pos, dict) and _norm_code(pos.get("code")) == code:
                found = True
                days += 1
                break
        if not found:
    ...
    for date in sorted(positions_map.keys(), reverse=True):
        ...
        for pos in hold_list if isinstance(hold_list, list) else []:
            if not isinstance(pos, dict) or _norm_code(pos.get("code")) != code:
                continue
            px = _safe_float(pos.get("price") or pos.get("last_price") or pos.get("current_price"))
            if px <= 0:
                return streak
            if px < ma20:
                streak += 1
...
def _check_holding_management(hold_list: list[dict[str, Any]], history: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=min(8, max(1, len(codes)))) as pool:
        futs = {pool.submit(fetch_jrj_daily_kline, code, 30): code for code in codes if code}
        for fut in as_completed(futs):
            code = futs[fut]
            try:
    ...
    for pos in hold_list:
        ...
        closes = [_safe_float(x.get("close")) for x in bars if _safe_float(x.get("close")) > 0]
        current_price = _safe_float(pos.get("price") or pos.get("last_price") or pos.get("current_price"))
        ...
        below_streak = _below_ma20_streak_from_history(code, current_price, history, ma20)
        if below_streak >= 3:
            flaws.append(_flaw(category="holding_flaw", severity="error", code=code, name=name, message="未执行MA20止损", detail={"below_ma20_days": below_streak}))
        elif below_streak >= 1:
            flaws.append(_flaw(category="holding_flaw", severity="info", code=code, name=name, message="逼近MA20止损线", detail={"below_ma20_days": below_streak}))

        if ma5 > 0:
            deviation = (current_price - ma5) / ma5
            if deviation > 0.20:
                flaws.append(_flaw(category="holding_flaw", severity="error", code=code, name=name, message="偏离MA5超20%未减仓", detail={"deviation_pct": round(deviation * 100, 2)}))
            elif deviation > 0.15:
                flaws.append(_flaw(category="holding_flaw", severity="warning", code=code, name=name, message="偏离MA5超15%未减仓", detail={"deviation_pct": round(deviation * 100, 2)}))
        ...
        if hold_earn_pct < -10 and _holding_days_from_history(code, history) > 5:
            flaws.append(_flaw(category="holding_flaw", severity="warning", code=code, name=name, message="持续亏损持仓未处理", detail={"hold_earn_pct": round(hold_earn_pct, 2)}))
...
def _check_discipline(
    ...
    if account_mode in {"defensive", "critical"}:
        has_buy = any(_order_direction(o) == "buy" and not _is_cancelled(o) for o in orders if isinstance(o, dict))
        if has_buy:
            flaws.append(
                _flaw(
                    category="discipline_flaw",
                    severity="error",
                    message="防御模式下加仓" if account_mode == "defensive" else "危急模式下仍加仓",
    ...
    if isinstance(history_orders, dict):
        for _, snapshot in sorted(history_orders.items(), reverse=True)[:3]:
            order_list = snapshot.get("order_list") if isinstance(snapshot, dict) else []
    ...
    for code, dirs in recent_codes.items():
        if "buy" in dirs and "sell" in dirs:
            flaws.append(_flaw(category="discipline_flaw", severity="warning", code=code, message="频繁翻转交易"))

    current_hold_list = history.get("current_hold_list") if isinstance(history, dict) else None
...
def _generate_suggestions(flaws: list[dict[str, Any]], timing_scores: list[dict[str, Any]]) -> list[str]:
    suggestions: list[str] = []
    unplanned = [f for f in flaws if f.get("category") == "unplanned_trade"]
    if unplanned:
        suggestions.append(f"当日存在{len(unplanned)}笔计划偏离买入，建议严格按推荐列表执行")
    holding = [f for f in flaws if f.get("category") == "holding_flaw" and "MA20" in str(f.get("message"))]
    if holding:
        target = holding[0]
        if target.get("name"):
            suggestions.append(f"持仓「{target['name']}」触发 MA20 风险提示，明日优先检查止损纪律")
    low_grades = [s for s in timing_scores if s.get("grade") in {"C", "D"}]
    if low_grades:
        suggestions.append(f"择时评分较弱交易 {len(low_grades)} 笔，建议复盘分时位置与 VWAP 偏离")
    if not suggestions:
...
def _append_evolution_feedback(result: dict[str, Any], base_dir: Path) -> None:
    ...
    try:
        ...
        buy_grades = [s.get("grade") for s in timing_scores if s.get("direction") == "buy" and s.get("grade")]
        sell_grades = [s.get("grade") for s in timing_scores if s.get("direction") == "sell" and s.get("grade")]
        ...
        major = next((f for f in result.get("flaws", []) if f.get("severity") in {"error", "warning"}), None)
...
def run_trade_review(
    ...
    review_date = str(broker_data.get("date") or "") or str(broker_data.get("fetched_at") or "")[:10] or _today_cn()
    ...
    orders = broker_data.get("order_list") if isinstance(broker_data.get("order_list"), list) else []
    positions = {
    ...
    regime = str(decision.get("market_regime") or "neutral") if isinstance(decision, dict) else "neutral"

    unplanned_flaws = _check_unplanned_trades(orders, candidates)
    missed_flaws = _check_missed_execution(orders, candidates, positions) if candidates else []
    timing_flaws, timing_scores = _analyze_timing(orders) if orders else ([], [])
    position_flaws, position_check = _check_position_compliance(positions, strategy, regime)
    ...
    buy_orders = [o for o in orders if _order_direction(o) == "buy" and not _is_cancelled(o)]
    sell_orders = [o for o in orders if _order_direction(o) == "sell" and not _is_cancelled(o)]
    cancelled_orders = [o for o in orders if _is_cancelled(o)]
    unplanned_buys = [f for f in unplanned_flaws if f.get("severity") in {"info", "warning", "error"}]
    planned_buys = max(0, len(buy_orders) - len(unplanned_buys))
    missed_buys = len([f for f in missed_flaws if f.get("category") == "missed_execution"])
    ...
    result = {
        ...
        "account_snapshot": {
            ...
            "account_mode": account_mode,
            "account_pnl_pct": round((hold_earn / total_assets * 100.0) if total_assets > 0 else 0.0, 2),
        },
        "execution_summary": {
        ...
        "metadata": {
            ...
            "decision_log_date": decision.get("as_of_date") if isinstance(decision, dict) else None,
            "decision_log_run_id": decision.get("run_id") if isinstance(decision, dict) else None,
            "data_completeness": {
                "broker_data": True,
...
def main() -> None:
    ...
    parser.add_argument("--decision-log", default=".memory/decision_log.jsonl")
    ...
    if args.pretty:
        ...
        print(json.dumps({"ok": "error" not in result, "output": args.output}, ensure_ascii=False))
