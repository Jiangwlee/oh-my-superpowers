...
# A machine-id must exist before Chromium/dbus start. Base images may ship an EMPTY
# /etc/machine-id (0 bytes), and an empty machine-id makes dbus/Chromium refuse to
...
# Login state lives on the mounted volume (Chromium profile).
mkdir -p /data/profile

# The profile is a persistent named volume, so Chromium's Singleton* lock files
# survive `docker compose down`. They bind to the previous container's hostname;
# on recreate the new hostname no longer matches and Chromium deadlocks on startup.
# These are pure single-instance locks (not login state — cookies/storage are
# separate files), so clearing stale ones at boot is safe: no Chromium is running
...
# nothing here is a durable SoT (ADR 0056). Chromium is pointed at it via
