...
> 静态 HTML artifact 发布使用 `omp html-serve publish <file.html> --to <relative/path.html>`；它写入 `HTML_SERVE_DATA_DIR` 并返回 `localhost_url` 与 `tailscale_url`。publish 可附带 `--title/--tag/--source` 元数据，记录在 `$OMP_HOME/runtime/html-serve/manifest.jsonl`；检索已发布内容用 `omp html-serve list [--under DIR] [--since/--until YYYY-MM-DD] [--grep REGEX] [--tag TAG]`，历史文件用 `omp html-serve reindex` 一次性重建 manifest。
