...
class SharedHelpersTest(unittest.TestCase):
    ...
    def test_now_cn_helpers(self) -> None:
        self.assertIn("T", shared.now_cn_iso())
        self.assertEqual(len(shared.today_cn()), 10)

    def test_position_market_value(self) -> None:
        self.assertEqual(shared.position_market_value({"market_value": 123.4}), 123.4)
        self.assertEqual(
            shared.position_market_value({"qty": 100, "last_price": 10.0}),
            1000.0,
        )

    def test_holding_days_from_history(self) -> None:
        history = {
            "positions": {
                "2026-02-24": {"hold_list": [{"code": "000001"}]},
                "2026-02-23": {"hold_list": [{"code": "SZ000001"}]},
                "2026-02-22": {"hold_list": []},
            }
        }
        self.assertEqual(shared.holding_days_from_history("000001", history), 2)

    def test_load_strategy_returns_deep_copy(self) -> None:
        first = shared.load_strategy("__missing__.yaml")
        first["trend_stock"]["position"] = "mutated"
        second = shared.load_strategy("__missing__.yaml")
        self.assertNotEqual(first["trend_stock"]["position"], second["trend_stock"]["position"])

    def test_enrich_hold_list_prices(self) -> None:
        hold_list = [{"code": "000001", "qty": 100}]

        def _fake_fetch(code: str, range_num: int) -> list[dict]:
            self.assertEqual(range_num, 5)
            return [{"close": 10.5}]

        logger = mock.Mock()
        shared.enrich_hold_list_prices(hold_list, fetch_daily_kline=_fake_fetch, logger=logger)

        self.assertEqual(hold_list[0]["last_price"], 10.5)
        self.assertEqual(hold_list[0]["market_value"], 1050.0)
