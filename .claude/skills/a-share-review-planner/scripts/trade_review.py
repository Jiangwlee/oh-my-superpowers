...
def _position_market_value(pos: dict[str, Any]) -> float:
    ...
    if mv > 0:
        return mv
    # JVQuant 返回格式无 market_value，用 hold_vol * 现价/成本 估算
    vol = _safe_float(pos.get("hold_vol") or pos.get("volume") or pos.get("qty"))
    price = _safe_float(
        pos.get("last_price")
...
def _enrich_hold_list_prices(hold_list: list[dict[str, Any]]) -> None:
    ...
    codes = [_norm_code(p.get("code")) for p in need_price if _norm_code(p.get("code"))]
    if not codes:
        return

    daily_map: dict[str, list[dict[str, Any]]] = {}
    with ThreadPoolExecutor(max_workers=min(8, len(codes))) as pool:
        futs = {pool.submit(fetch_jrj_daily_kline, code, 5): code for code in codes}
        for fut in as_completed(futs):
            code = futs[fut]
            try:
    ...
    for pos in need_price:
        ...
        if not bars:
            continue
        closes = [
            _safe_float(b.get("close")) for b in bars if _safe_float(b.get("close")) > 0
        ]
        if closes:
            latest_price = closes[-1]
            pos["last_price"] = latest_price
            vol = _safe_float(
                pos.get("hold_vol") or pos.get("volume") or pos.get("qty")
            )
            if vol > 0:
